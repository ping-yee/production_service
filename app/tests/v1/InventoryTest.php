...
class InventoryTest extends DatabaseTestCase
{
	...
	public function testAddInventory()
	{
		...
		/** 空資料輸入*/
		$otherDataNotExistResults = $this->withBodyFormat('json')->post('api/v1/inventory/addInventory', []);
		...
		$keyNotExistResults = $this->withBodyFormat('json')->post('api/v1/inventory/addInventory', $keyNotExistData);
		...
		$dataRepeatResults = $this->withBodyFormat('json')->post('api/v1/inventory/addInventory', $dataRepeatData);
		...
		$orderIsNotCreateDataResults = $this->withBodyFormat('json')->post('api/v1/inventory/addInventory', $orderIsNotCreateData);
		$orderIsNotCreateDataResults->assertStatus(400);
		...
		$beforeInventory = $inventoryModel->where('p_key', 1)->first();

		$SuccessfulResults = $this->withBodyFormat('json')->post('api/v1/inventory/addInventory', $SuccessfulData);
		if (!$SuccessfulResults->isOK()) $SuccessfulResults->assertStatus(400);
		$SuccessfulResults->assertStatus(200);

		//get transation after data
		$afterInventory = $inventoryModel->where('p_key', 1)->first();

		// 將TRANSATION前後的資料進行相減比對
		$transationIsTrue = ($afterInventory->amount - $SuccessfulData['addAmount']) == $beforeInventory->amount;
		$SuccessfulResults->assertTrue($transationIsTrue);

		//於inventory中比對是否有該筆資料
		$seeingData = [
			"p_key"=> $insertIDArray[0],
			"amount"=> $SuccessfulData['addAmount'] + $beforeInventory->amount
		];
	...
	public function testReduceInventory()
	{
		...
		/** 傳入值不完整*/
		$otherDataNotExistResults = $this->withBodyFormat('json')->post('api/v1/inventory/reduceInventory', []);
		$otherDataNotExistResults->assertStatus(404);
		...
		$keyNotExistResults = $this->withBodyFormat('json')->post('api/v1/inventory/reduceInventory', $keyNotExistData);
		$keyNotExistResults->assertStatus(404);
		...
		$dataRepeatResults = $this->withBodyFormat('json')->post('api/v1/inventory/reduceInventory', $dataRepeatData);
		$dataRepeatResults->assertStatus(400);
		...
		$notFindInventoryResults = $this->withBodyFormat('json')->post('api/v1/inventory/reduceInventory', $notFindInventoryData);
		$notFindInventoryResults->assertStatus(404);

		/** 庫存數量不夠 start */
		$amountInsufficientData = [
			"p_key" => $insertIDArray[1],
			"o_key" => $okeyArray['okeyByProduct2'],
			"reduceAmount" => 9999999,
		];

		$amountInsufficientResults = $this->withBodyFormat('json')->post('api/v1/inventory/reduceInventory', $amountInsufficientData);
		...
		$successfulResults =$this->withBodyFormat('json')->post('api/v1/inventory/reduceInventory', $successfulData);
		if (!$successfulResults->isOK()) $successfulResults->assertStatus(400);
		$successfulResults->assertStatus(200);
		...
		$transationIsTrue = ($afterInventory->amount + $successfulData['reduceAmount']) == $beforeInventory->amount;
		$successfulResults->assertTrue($transationIsTrue);

		/**於inventory中比對是否有該筆資料*/
		$seeingData = [
			"p_key" => $insertIDArray[1],
			"amount" => $successfulData['reduceAmount'] - $beforeInventory->amount
		];
